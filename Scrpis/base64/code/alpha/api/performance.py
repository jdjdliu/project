...
from alpha.schemas import CreateFundPerformanceSchema, FundPerformanceSchema, CreateIndexPerformanceSchema, CreatePerformanceSchema, IndexPerformanceSchema, PerformanceSchema
...
# ! 这个接口要对用户屏蔽
@router.post("/fund", response_model=ResponseSchema[FundPerformanceSchema])
async def update_fund_backtest_performance(
    task_id: UUID,
    request: CreateFundPerformanceSchema,
    credential: Credential = Depends(auth_required()),
) -> ResponseSchema[FundPerformanceSchema]:
    obj: Union[Alpha, Backtest]
    ...
    await Performance.filter(alpha_id=obj.id, source=request.source, run_datetime=request.run_datetime).delete()
    fund_performance = await Performance.create(alpha_id=obj.id, **request.dict())

    return ResponseSchema(data=FundPerformanceSchema.from_orm(fund_performance))