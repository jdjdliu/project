...
from alpha.constants import AlphaType, AuditStatus, PerformanceSource, AlphaBuildType, ProductType
...
async def get_access_alpha(
    ...
    alphas = await Alpha.filter(id__in=alpha_ids, alpha_type=AlphaType.ALPHA, catalog_id__isnull=True, product_type=ProductType.STOCK).only("alpha_id", "name")
...
async def get_access_alpha_(
    ...
    alphas = await Alpha.filter(id__in=alpha_ids, alpha_type=AlphaType.ALPHA, catalog_id__isnull=True, product_type=ProductType.STOCK).only("name", "expression")
