...
from alpha.schemas import (BacktestSchema, CompositionAlpha,
                           CreateAlphaFromBacktestRequest,
                           CreateIndexBacktestRequest)
from sdk.task import TaskStatus
from settings import (ALPHA_HOST, HEADERS, INDEX_PARAMETER, PUBLIC_REPO,
                      TASK_HOST, TORTOISE_ORM, alpha_file, alpha_name,
                      backtest_file, creator_id, final_mapping)
...
class BacktestMethod:
    ...
    async def create_backtest(cls: Type["BacktestMethod"]):
        ...
        for expression, name in alpha_name.items():
            alpha = await Alpha.get_or_none(name=name, catalog_id__isnull=False)
            if not alpha:
