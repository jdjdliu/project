...
from alpha.schemas import CompositionAlpha, CreateAlphaFromBacktestRequest, CreateIndexBacktestRequest
...
class BacktestMethod:
    ...
    def create_backtest(cls: Type["BacktestMethod"]):
        ...
        backtest_ids = {}
        alphas = cls._get_catalog_alpha()

        for alpha in alphas:
            expression = alpha["alphas"][0]["alpha"]
            data = CreateIndexBacktestRequest(
                name=alpha["name"],
                column="index",
                alphas=[CompositionAlpha(alpha=expression, weight=1)],
                parameter=format_index_paramter(alpha["alpha_id"]),
                alpha_type="INDEX",
                product_type="STOCK",
                description="",
            )
            ...
            print(f"{cls.__name__}.{sys._getframe().f_code.co_name}  |  {backtest['id']}, {backtest['name']} 已写入 ‘构建中的指数’.")

        with open(backtest_file, "w") as f:
            f.write(json.dumps(backtest_ids))

    @classmethod
    def _get_catalog_alpha(cls: Type["BacktestMethod"]):
        url = f"{ALPHA_HOST}/api/alpha/catalog/alpha"
        resp = requests.get(url)
        alphas = []
        for cata in json.loads(resp.text)["data"]:
            alphas.extend(cata["alphas"])
        return alphas
...
if __name__ == "__main__":
    ...
    AlphaMethod.put_alpha()  # 从backtest提交并更新catalog & 添加index到repo
