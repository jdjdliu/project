...
import numpy as np
import pandas as pd
from sdk.datasource import DataReaderV2, IndustryDefs
from sdk.datasource.bigdata.constants import merged_data_dir
from sdk.sdk.io.file import data_path
...
def load_factor_data(start_date, end_date):
    ...
    for year in range(pd.to_datetime(start_date).year, pd.to_datetime(end_date).year + 1):
        ...
        file_path = data_path(merged_data_dir, "factor_data.CN_STOCK_A.__T_y_%s__.h5" % year)
...
def factor_analyze(perf_df):
    ...
    def compute_factor(series):
        cur_factor_data = factor_data.ix[series.name.date()]
        ...
        for idx, item in enumerate(series.positions):
            ...
            values += cur_factor_data.ix[instrument].values * weight
            weight_sum += weight
        values /= weight_sum
...
def industry_analyze(perf_df):
    ...
    def compute_industry_factor(series):
        cur_factor_data = industry_data.ix[series.name.date()]
        ...
        for idx, item in enumerate(series.positions):
            ...
            if instrument not in cur_factor_data.index:
                continue
            weight = item["last_sale_price"] * item["amount"]
            industry_id = cur_factor_data.ix[instrument].industry_sw_level1
            if industry_id == 0:
                continue
