...
class FeaturePerformance:
    def __init__(self,start_date,end_date,rebalance_period,
                buy_commission_rate,sell_commission_rate,ic_method,quantile_num,is_standardlize,is_winsorize
                ):
    ...
    def ols_stats_processing(self, merge_df, factor_name):
        ...
        T.log.info("ols_stats_processing process  %.3fs" %
    ...
    def group_processing(self, merge_df, base_factor_df, factor_name):
        ...
        def _cut_box(df, quantile_num=5):
            if df.factor.isnull().sum() == len(df):  # 因子值全是nan的话
                ...
                df["factor_group"] = pd.qcut(    # qcut 改成cut
        ...
        def _get_stats(results, col_name):
            ...
            for n in [1, 5, 22]:
                res['近{}收益率'.format(date_dict[n])] = np.round(
                    results[col_name.replace('ret', 'pv')][-1] / results[col_name.replace('ret', 'pv')][
                        -(n + 1)] - 1, 4)
            res.update({
        ...
        T.log.info("group_processing process  %.3fs" %
                   (time.time() - start_time))
