...
# log = logbook.Logger('random_forest_classifier')
log = BigLogger("random_forest_classifier")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\分类"
bigquant_friendly_name = "随机森林-分类"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    feature_fraction: I.float("特征使用率：寻找最佳分割时要考虑的特征比率", 0.0, 1.0) = 1.0,
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = feature_train(
        training_ds=training_ds,
        features=features,
        train_algorithm=RandomForestClassifier,
