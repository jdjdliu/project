...
# log = logbook.Logger('cluster_birch')
log = BigLogger("cluster_birch")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\聚类"
bigquant_friendly_name = "Birch-聚类"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    compute_labels: I.bool("输出时表示类别，是否标示类别输出，默认为True") = True,
    threshold: I.float("簇半径阈值，限制新加入的样本和存在与现有子集群中样本的最大距离，默认为0.5") = 0.5,
    branching_factor: I.int("分支因子，限制一个节点中的子集群的数量，默认是50", min=2) = 50,
    key_cols: I.str("关键列，关键列的数据会复制到预测结果里，多个列用英文逗号分隔") = "date,instrument",
    other_train_parameters: I.code(
        "其他训练参数，字典格式，例:{'criterion': 'mse'}", I.code_python, "{'n_clusters': 3}", specific_type_name="字典", auto_complete_type=""
    ) = {"n_clusters": 3},
) -> [
    ...
    if training_ds:
        ...
        result_outputs = M.cached.v2(
            ...
            kwargs=dict(
                ...
                features=features,
                train_parameters=dict(compute_labels=compute_labels, threshold=threshold, branching_factor=branching_factor),
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = cluster_train(
        training_ds=training_ds, features=features, train_algorithm=Birch, train_parameters=dict(train_parameters, **other_train_parameters)
    )
    return outputs
...
if __name__ == "__main__":
    ...
    m2 = M.cluster_birch.v1(training_ds=m1.data_1, features=m3.data, predict_ds=m1.data_1, threshold=-1, other_train_parameters={}, m_cached=False)
