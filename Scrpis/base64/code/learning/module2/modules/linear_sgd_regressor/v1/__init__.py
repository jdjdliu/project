...
# log = logbook.Logger('linear_sgd_regressor')
log = BigLogger("linear_sgd_regressor")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\回归"
bigquant_friendly_name = "线性随机梯度下降-回归"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    loss: I.choice("损失函数", values=["squared_loss", "huber", "epsilon_insensitive", "squared_epsilon_insensitive"]) = "huber",
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = train(
        training_ds=training_ds, features=features, train_algorithm=SGDRegressor, train_parameters=dict(train_parameters, **other_train_parameters)
    )
    return outputs
