...
# log = logbook.Logger('preprocessing_max_abs_scaler')
log = BigLogger("preprocessing_max_abs_scaler")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\特征预处理"
bigquant_friendly_name = "最大绝对值归一化"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    if training_ds:
        ...
        train_outputs = M.cached.v2(
            run=transform, kwargs=dict(training_ds=training_ds, train_algorithm=MaxAbsScaler, features=features), m_silent=True
        )
    ...
    if predict_ds:
        transform_predictds = M.cached.v2(
            run=transform, kwargs=dict(training_ds=predict_ds, train_algorithm=MaxAbsScaler, model=model), m_silent=True
        ).transform_ds
    else:
...
if __name__ == "__main__":
    ...
    m2 = M.preprocessing_max_abs_scaler.v1(training_ds=m1.data_1, features=m3.data, predict_ds=m1.data_1, m_cached=False)
    print(1)
