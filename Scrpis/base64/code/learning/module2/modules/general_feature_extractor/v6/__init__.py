...
class BigQuantModule:
    def __init__(
        ...
        before_start_days: I.float("向前取数据天数，比如，用户通过表达式计算的衍生特征，可能需要用到开始日期之前的数据，可以通过设置此值实现，则数据将从 开始日期-向前取数据天数 开始取。考虑到节假日等，建议将此值得大一些") = 0,
    ) -> [I.port("基础特征数据", "data")]:
        """
        ...
        self.__before_start_days = before_start_days + 10
        self.__features = bigexpr.extract_variables(smart_list(features))
