...
# log = logbook.Logger('cluster_kmeans')
log = BigLogger("cluster_kmeans")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\聚类"
bigquant_friendly_name = "K均值-聚类"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    n_clusters: I.int("簇的个数，聚类中心数量，默认为8") = 8,
    init: I.choice("质心初始化方法，默认为k-means++", values=["k-means++", "random"]) = "k-means++",
    n_init: I.int("质心初始化次数，初始化质心的次数，默认为10") = 10,
    iterations: I.int("最大迭代次数，迭代得到最终质心的次数", min=1) = 300,
    algorithm: I.choice("K-Means算法，auto选择elkan表示密集数据，full表示稀疏数据", values=["auto", "full", "elkan"]) = "auto",
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = cluster_train(
        training_ds=training_ds, features=features, train_algorithm=KMeans, train_parameters=dict(train_parameters, **other_train_parameters)
    )
    return outputs
