...
# log = logbook.Logger('adaboost_regressor')
log = BigLogger("adaboost_regressor")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\回归"
bigquant_friendly_name = "自适应提升树-回归"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    loss: I.choice("损失函数，每次增强迭代后更新权重时使用的损失函数，默认为线性", values=["linear", "square", "exponential"]) = "linear",
    ...
    if training_ds:
        ...
        model = M.cached.v2(
            ...
            kwargs=dict(
                ...
                features=features,
                train_parameters=dict(n_estimators=n_estimators, learning_rate=learning_rate, loss=loss),
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = train(
        training_ds=training_ds,
        features=features,
        train_algorithm=AdaBoostRegressor,
