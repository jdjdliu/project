...
# log = logbook.Logger('decision_tree_regressor')
log = BigLogger("decision_tree_regressor")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\回归"
bigquant_friendly_name = "决策树-回归"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    criterion: I.choice("决定分割的标准，支持均方误差mse，平均绝对误差mae及弗里德曼改进均方误差friedman_mse。默认是mse", values=["mse", "mae", "friedman_mse"]) = "mse",
    ...
    if training_ds:
        ...
        model = M.cached.v2(
            ...
            kwargs=dict(
                ...
                features=features,
                train_parameters=dict(criterion=criterion, max_features=feature_fraction, max_depth=max_depth, min_samples_leaf=min_samples_per_leaf),
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = feature_train(
        training_ds=training_ds,
        features=features,
        train_algorithm=DecisionTreeRegressor,
