...
bigquant_friendly_name = "Cropping2D层"
bigquant_doc_url = "https://bigquant.com/docs/"


def bigquant_run(
    cropping: I.str("cropping，长为4的列表，2个一组，分别为宽和高方向上头部与尾部需要裁剪掉的元素数。列表用英文逗号(,)分隔的整数列表，例如 10 或者 32,24") = "0,0,0,0",
    ...
    if len(cropping) != 4:
        raise Exception("cropping：必须为4个整数")
    cropping = ((cropping[0], cropping[1]), (cropping[2], cropping[3]))

    from tensorflow.keras.layers import Cropping2D

    layer = Cropping2D(cropping=cropping, data_format=data_format)
