...
# log = logbook.Logger('linear_regression')
log = BigLogger("linear_regression")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\回归"
bigquant_friendly_name = "线性-回归"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    fit_intercept: I.bool("是否计算该模型的截距") = True,
    normalize: I.bool("回归X是否被规范化，如果为True，回归X被规范化") = False,
    ...
    if training_ds:
        ...
        model = M.cached.v2(
            ...
            kwargs=dict(
                ...
                features=features,
                train_parameters=dict(fit_intercept=fit_intercept, normalize=normalize, n_jobs=workers),
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = train(
        training_ds=training_ds,
        features=features,
        train_algorithm=LinearRegression,
