...
from learning.settings import CUSTOM_MODULE_COLLECTION, CUSTOM_MODULE_IMP_FILE, jpy_user_name
from sdk.db.mongo import MongoClient

mongo_client = MongoClient.get_collection(CUSTOM_MODULE_COLLECTION)
...
def format_custom_module(name: str) -> str:
    _item = mongo_client.find({"name": name}).sort("version", -1).limit(1)
    if not _item:
        raise ValueError("can't find module {name}")

    code = f"\nfrom learning.module2.common.custom_module_imp import *\n"
    item: Dict = [i for i in _item][0]
    code += item["data"]["main_func"]
    code += item["data"]["post_func"]
    return code
