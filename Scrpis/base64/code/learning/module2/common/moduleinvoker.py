...
from learning.module2.common.utils import format_custom_module
...
class ModuleInvoker:
    ...
    def load(name, version, kwargs={}, custom_module=False):
        ...
        except ImportError:  # TODO: [vv] get source code from mongo, import module from monoo
            import imp
            code = format_custom_module(name)
            custom_module = imp.new_module(name)
            version_custom_module = imp.new_module(f"v{version}")
            exec(code, version_custom_module.__dict__)
            setattr(custom_module, version, version_custom_module)                                                              
            pkg = version_custom_module
