...
class FundAlphaParameter(BaseModel):
    start_date: str = Field(ALPHA_STRATE_DATE, description="回测开始日期")
    end_date: str = Field(default_factory=lambda: date.today().strftime("%Y-%m-%d"), description="回测开始日期")
    rebalance_period: int = Field(22, description="调仓周期")
    buy_commission_rate: float = Field(0.001, gt=0.0, description="买入费用")
    sell_commission_rate: float = Field(0.001, gt=0.0, description="卖出费用")
    ic_method: str = Field("Rank_IC", description="IC计算方式")
    quantile_num: int = Field(5, ge=1, description="分组数量")
    is_standardlize: bool = Field(True, description="是否标准化")
    is_winsorize: bool = Field(False, description="是否极值处理")
