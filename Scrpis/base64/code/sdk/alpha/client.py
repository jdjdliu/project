...
class AlphaClient:
    ...
    @classmethod
    def create_fund_backtest_performance(
        cls: Type["AlphaClient"], task_id: str, request: CreateFundPerformanceSchema, credential: Credential
    ) -> FundPerformanceSchema:
        return FundPerformanceSchema(
            **cls.HTTPClient.post("/api/alpha/performance/fund", params={"task_id": task_id}, payload=request.dict(), credential=credential)
        )
