...
#       desc='BETA值(所在行业)，timeperiod=i (历史数据,行业按照上市日期固定, 增量数据按照当前行业计算)',
#       rank=True, transform_for_ranker='x * 100', base_field=BAR_FIELDS),
#     H('beta_industry1_$i_0', range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("INDUSTRY" ,timeperiod=i)',
...
_FEATURES = __make([
    ...
    'tables:bar1d,stock_status',
    'table_name:features_bar1d',
    F('open', R_0, 'df.open.shift(i)', '第前i个交易日的开盘价，当天为0', transform_for_ranker='x*100', base_field='open'),
    F('high', R_0, 'df.high.shift(i)', '第前i个交易日的最高价', transform_for_ranker='x*100', base_field='high'),
    F('low', R_0, 'df.low.shift(i)', '第前i个交易日的最低价', transform_for_ranker='x*100', base_field='low'),
    F('volume', R_0, 'df.volume.shift(i)', '第前i个交易日的交易量', transform_for_ranker='x/10000', base_field='volume'),
    F('adjust_factor', R_0, 'df.adjust_factor.shift(i)', '第前i个交易日的复权因子', transform_for_ranker='x*10',
      base_field='adjust_factor',
      markets='CN_STOCK_A,CN_FUND,US_STOCK,HK_STOCK'),
    F('deal_number', R_0, 'df.deal_number.shift(i)', '第前i个交易日的成交笔数', transform_for_ranker=None, markets='CN_STOCK_A',
      base_field='deal_number'),
    F('price_limit_status', R_0, 'df.price_limit_status.shift(i)', '第前i个交易日的股价在收盘时的涨跌停状态，1表示跌停，2表示未涨跌停，3则表示涨停',
      base_field='price_limit_status',
      transform_for_ranker=None, markets='CN_STOCK_A,CN_FUND'),

    F('close', R_0, 'df.close.shift(i)', '第前i个交易日的收盘价', transform_for_ranker='x*100', base_field='close'),
    F('daily_return', R_0, 'df.close.shift(i) / df.close.shift(i + 1)', '第前i个交易日的收益，=close_i/close_(i+1)',
      base_field='close',
      transform_for_ranker='x*10000'),
    F('return', R_0, 'df.close / df.close.shift(i + 1)', '过去i个交易日的收益，=close_0/close_(i+1)', base_field='close',
      transform_for_ranker='x*10000'),
    F('rank_return', R_0, None, '过去i个交易日的收益 (return_i) 排名，=从小到大排名序号/总数', transform_for_ranker='x*10000'),

    F('amount', R_0, 'df.amount.shift(i)', '第前i个交易日的交易额', transform_for_ranker='x/10000', base_field='amount'),
    F('rank_amount', R_0, None, '第前i个交易日的交易额百分比排名', transform_for_ranker='x*10000'),
    F('avg_amount', R_0, 'df.amount.rolling(i + 1).mean()', '过去i个交易日的平均交易额，0表示今日', transform_for_ranker='x/10000',
      base_field='amount'),
    F('rank_avg_amount', R_0, None, '过去i个交易日的平均交易额，百分比排名', transform_for_ranker='x*10000'),

    'category:换手率因子',
    'markets:CN_FUND,CN_STOCK_A,US_STOCK,HK_STOCK',
    'tables:bar1d',
    'table_name:features_turn',
    F('turn', R_0, 'df.turn.shift(i)', '第前i个交易日的换手率', transform_for_ranker='x*10000', base_field='turn'),
    F('rank_turn', R_0, None, '过去i个交易日的换手率 (turn_i) 排名，=从小到大排名序号/总数', transform_for_ranker='x*10000'),
    F('avg_turn', R_0, 'df.turn.rolling(i+1).mean()', '过去i个交易日的平均换手率，0表示今日', transform_for_ranker='x*10000',
      base_field='turn'),
    F('rank_avg_turn', R_0, None, '过去i个交易日的平均换手率排名，=从小到大排名序号/总数', transform_for_ranker='x*10000'),
    ...
    *[
    ...
    H('roa2', 'df.roa2', transform_for_ranker=None),
    H('roa', 'df.roa', transform_for_ranker=None),
    H('op_gr','df.op_gr',  transform_for_ranker=None),
    H('pro_tog','df.pro_tog',  transform_for_ranker=None),
    H('gc_gr', 'df.gc_gr', transform_for_ranker=None),
    H('ebit_gr', 'df.ebit_gr', transform_for_ranker=None),
    H('gro_pro_mar', 'df.gro_pro_mar', transform_for_ranker=None),
    H('net_pro_mar', 'df.net_pro_mar', transform_for_ranker=None),
    H('cog_sal', 'df.cog_sal', transform_for_ranker=None),

    H('yoy_eps_bas', 'df.yoy_eps_bas', transform_for_ranker=None),
    H('yoy_eps_dil', 'df.yoy_eps_dil', transform_for_ranker=None),
    H('yoy_ocf', 'df.yoy_ocf', transform_for_ranker=None),
    H('yoy_op', 'df.yoy_op', transform_for_ranker=None),
    H('yoy_ebt_tot', 'df.yoy_ebt_tot', transform_for_ranker=None),
    H('yoy_net_pro', 'df.yoy_net_pro', transform_for_ranker=None),
    H('yoy_tr', 'df.yoy_tr', transform_for_ranker=None),
    H('yoy_or', 'df.yoy_or', transform_for_ranker=None),
    H('yoy_bps', 'df.yoy_bps', transform_for_ranker=None),
    H('yoy_ass', 'df.yoy_ass', transform_for_ranker=None),
    H('yoy_equ_sha', 'df.yoy_equ_sha', transform_for_ranker=None),

    H('acc_lia_pro', 'df.acc_lia_pro', transform_for_ranker=None),
    H('ass_equ', 'df.ass_equ', transform_for_ranker=None),
    H('lia_tot_equ', 'df.lia_tot_equ', transform_for_ranker=None),
    H('ebit_int', 'df.ebit_int', transform_for_ranker=None),
    H('cur_rat', 'df.cur_rat', transform_for_ranker=None),
    H('qui_rat', 'df.qui_rat', transform_for_ranker=None),
    H('cas_rat', 'df.cas_rat', transform_for_ranker=None),
    H('cat_ass', 'df.cat_ass', transform_for_ranker=None),
    H('ocf_lia_cur', 'df.ocf_lia_cur', transform_for_ranker=None),
    H('equ_deb', 'df.equ_deb', transform_for_ranker=None),
    H('ocf_deb', 'df.ocf_deb', transform_for_ranker=None),

    H('inv_rn', 'df.inv_rn', transform_for_ranker=None),
    H('art_rn', 'df.art_rn', transform_for_ranker=None),
    H('art_day', 'df.art_day', transform_for_ranker=None),
    H('fat_rn', 'df.fat_rn', transform_for_ranker=None),
    H('cat_rn', 'df.cat_rn', transform_for_ranker=None),
    H('ass_rn', 'df.ass_rn', transform_for_ranker=None),
    H('fcff', 'df.fcff', transform_for_ranker=None),
    H('wor_cap', 'df.wor_cap', transform_for_ranker=None),
    H('net_wor_cap', 'df.net_wor_cap', transform_for_ranker=None),

    H('eps_bas', 'df.eps_bas', transform_for_ranker=None),
    H('eps_dlt', 'df.eps_dlt', transform_for_ranker=None),
    H('bps', 'df.bps', transform_for_ranker=None),
    H('ocfps', 'df.ocfps', transform_for_ranker=None),
    H('gr_ps', 'df.gr_ps', transform_for_ranker=None),
    H('orps', 'df.orps', transform_for_ranker=None),
    H('sur_cap', 'df.sur_cap', transform_for_ranker=None),
    H('sur_res', 'df.sur_res', transform_for_ranker=None),
    H('und_ps', 'df.und_ps', transform_for_ranker=None),
    H('ret_ps', 'df.ret_ps', transform_for_ranker=None),
    H('cfps', 'df.cfps', transform_for_ranker=None),
    H('ebit_ps', 'df.ebit_ps', transform_for_ranker=None),
    H('fcff_ps', 'df.fcff_ps', transform_for_ranker=None),
    H('fcfe_ps', 'df.fcfe_ps', transform_for_ranker=None),
...
class FeatureDefs:
    ...
    # FEATURE_DAYS_LOOK_BACK = int((max(max(f.range) if f.range else 0 for f in FEATURE_LIST if not f.field.startswith('rank_')) + 1)*1.5)  # noqa
    max_range = 0
...
class FeatureContext:
    ...
    def ta(self, ta_func_name, *args):
        ...
        if key not in self.__ta_cached_data:
            func = ta_abstract.Function(ta_func_name)
    ...
    def beta(self, base_name, timeperiod):
        ...
        ret = v_returns.rolling(window=timeperiod).cov(other=b_returns, pairwise=True) / (b_returns.rolling(center=False, window=timeperiod).std() ** 2)  # noqa
