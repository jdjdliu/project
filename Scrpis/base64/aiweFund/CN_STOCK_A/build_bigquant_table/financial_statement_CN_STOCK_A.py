...
class BuildFinancial(Build):
    ...
    def run(self):
        back_date = (datetime.datetime.strptime(self.start_date, '%Y-%m-%d') - datetime.timedelta(days=365*2)).strftime('%Y-%m-%d')
        common_col = [
            'pty_cd',  # 主体内部编码，主键
            'date',    # 主要是公告日期rename得到的，非主键
            'end_dt',  # 报告截止日期 对应wind的report_period 主键
            'rpt_trm_typ',  # 报告期类型：对应wind  001-一季报，002-半年报，003-三季报，004-年报，999-其他；
        ...
        drvind_df = DataSource("PTY_FIN_DRVIND").read(t_pty_lst, start_date=back_date, end_date=self.end_date,
                                                      fields=read_source_info.get('PTY_FIN_DRVIND'))
        if not isinstance(drvind_df, pd.DataFrame):
            drvind_df = pd.DataFrame(columns=read_source_info.get('PTY_FIN_DRVIND'))
        del drvind_df['instrument']
        drvind_df['date'] = pd.to_datetime(drvind_df.date)
        drvind_df['end_dt'] = pd.to_datetime(drvind_df.end_dt)
        drvind_df = drvind_df.fillna(value=np.inf)

        income_df = DataSource("PTY_FIN_INCOME").read(t_pty_lst, start_date=back_date, end_date=self.end_date,
                                                      fields=read_source_info.get('PTY_FIN_INCOME'))
        if not isinstance(income_df, pd.DataFrame):
            income_df = pd.DataFrame(columns=read_source_info.get('PTY_FIN_INCOME'))
        del income_df['instrument']
        income_df['date'] = pd.to_datetime(income_df.date)
        income_df['end_dt'] = pd.to_datetime(income_df.end_dt)
        income_df = income_df.fillna(value=np.inf)

        balance_df = DataSource("PTY_FIN_BALANCE").read(t_pty_lst, start_date=back_date, end_date=self.end_date,
                                                        fields=read_source_info.get('PTY_FIN_BALANCE'))
        ...
        balance_df = balance_df.fillna(value=np.inf)

        res_df = pd.merge(drvind_df, income_df, on=['pty_cd', 'date', 'end_dt', 'rpt_trm_typ', 'rpt_typ'], how='outer')
        res_df = pd.merge(res_df, balance_df, on=['pty_cd', 'date', 'end_dt', 'rpt_trm_typ', 'rpt_typ'], how='outer')
        print('merged res_df shape: ', res_df.shape)

        res_df.loc[res_df['rpt_typ'] == '003', 'rpt_typ'] = '000'  # 多个报表类型以 003-合并报表（调整）为第一优先级，然后才是001-合并报表
        res_df.sort_values(['pty_cd', 'date', 'end_dt', 'rpt_typ'], inplace=True)
        res_df = res_df.drop_duplicates(['pty_cd', 'date', 'end_dt'], keep='first')
        res_df['acc_lia_pro'] = res_df.acc_lia_pro_cal
        res_lst = []
        for pty_cd, _df in res_df.groupby('pty_cd'):
            _df = _df.fillna(method='ffill')
            _df['pty_cd'] = pty_cd
            res_lst.append(_df)
        res_df = pd.concat(res_lst, ignore_index=True)
        res_df = res_df.replace(np.inf, None)
        res_df = res_df[res_df.date >= pd.to_datetime(self.start_date)]
        print('res_df ffill finish, cut date to start_date shape：', res_df.shape)

        del res_df['rpt_typ']

        stock_basic_df = DataSource('basic_info_CN_STOCK_A').read(fields=['stk_cd', 'instrument'])
        stock_source_df = DataSource('STK_BASICINFO').read(fields=['stk_cd', 'pty_cd'])[['stk_cd', 'pty_cd']]
        stock_basic_df = pd.merge(stock_basic_df, stock_source_df, on=['stk_cd'], how='inner')

        #         cal_df = res_df.copy()
        if 'instrument' in res_df.columns.tolist():
            del res_df['instrument']

        res_df = pd.merge(res_df, stock_basic_df, on=['pty_cd'], how='inner')  # add column: stk_cd, instrument

        res_df = res_df.drop_duplicates(['instrument', 'date', 'end_dt'], keep='last')
        if res_df.empty:
            return
        res_df = res_df.rename(columns={'end_dt': 'fs_quarter'})  # 'date': 'fs_publish_date',
        res_df['fs_quarter'] = res_df.fs_quarter.dt.strftime('%Y%m%d')  #

        res_df['fs_quarter_year'] = res_df['fs_quarter'].apply(lambda x: int(x[:4])).astype('int')
        res_df['fs_quarter_index'] = res_df['rpt_trm_typ'].map({'001': 1, '002': 2, '003': 3, '004': 4, '999': 9})
        res_df = res_df[res_df.fs_quarter_index.notnull()]

        res_df['suffix'] = res_df.instrument.apply(lambda x: x.split('.')[1])
        suffix = res_df.suffix.unique().tolist()
        assert {'SZA', 'SHA', 'BJA'} >= set(suffix), print(f'error suffix: {suffix}')
        # print(res_df)
        print(res_df.groupby(['fs_quarter_year']).instrument.count())
        res_df = res_df[list(self.schema['fields'].keys())]
        self._update_data(res_df)
