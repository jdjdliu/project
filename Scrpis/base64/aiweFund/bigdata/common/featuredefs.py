...
class _FEATURETUPE:
    def __init__(self, field, range, expr, desc, category, transform_for_ranker, markets='all'):
        self.field = field
        self.range = range
        self.expr = expr
        self.desc = desc
...
def F(field, range, expr, desc, transform_for_ranker, markets=None):
    if markets:
        return lambda category,default_markets: _FEATURETUPE(field, range, expr, desc, category, transform_for_ranker, markets)
    else:
        return lambda category,default_markets: _FEATURETUPE(field, range, expr, desc, category, transform_for_ranker, default_markets)


def H(field, expr=None, desc=None, rank=False, transform_for_ranker=None,markets=None, range=[0]):
...
def __make(items):
    ...
    for item in items:
        if isinstance(item, str):
            ...
            results.extend([f(category, markets) for f in item])
        else:
            results.append(item(category, markets))
    return results
...
_FEATURES = __make([
    ...
    'category:基本信息',
    'markets:CN_STOCK_A',
    F('list_days', R_0, '(df.date - df.list_date).dt.days', '已经上市的天数，按自然日计算', transform_for_ranker=None,markets='CN_STOCK_A,US_STOCK,HK_STOCK'),
    H('list_board', expr='df.list_board.map({"主板": 1, "中小企业板": 2, "创业板": 3})', desc='上市板，主板：1，中小企业板：2，创业板：3', transform_for_ranker=None),
    ...
    *[
    ...
    'category:量价因子',
    'markets:all',
    F('open', R_1, 'df.open.shift(i)', '第前i个交易日的开盘价，当天为0', transform_for_ranker='x*100'),
    F('high', R_1, 'df.high.shift(i)', '第前i个交易日的最高价', transform_for_ranker='x*100'),
    F('low', R_1, 'df.low.shift(i)', '第前i个交易日的最低价', transform_for_ranker='x*100'),
    F('volume', R_1, 'df.volume.shift(i)', '第前i个交易日的交易量', transform_for_ranker='x/10000'),
    F('adjust_factor', R_1, 'df.adjust_factor.shift(i)', '第前i个交易日的复权因子', transform_for_ranker='x*10', markets='CN_STOCK_A,CN_FUND,US_STOCK,HK_STOCK'),
    F('deal_number', R_1, 'df.deal_number.shift(i)', '第前i个交易日的成交笔数', transform_for_ranker=None, markets='CN_STOCK_A'),
    F('price_limit_status', R_1, 'df.price_limit_status.shift(i)', '第前i个交易日的股价在收盘时的涨跌停状态，1表示跌停，2表示未涨跌停，3则表示涨停', transform_for_ranker=None, markets='CN_STOCK_A,CN_FUND'),

    F('close', R_2, 'df.close.shift(i)', '第前i个交易日的收盘价', transform_for_ranker='x*100'),
    F('daily_return', R_3, 'df.close.shift(i) / df.close.shift(i + 1)', '第前i个交易日的收益，=close_i/close_(i+1)', transform_for_ranker='x*10000'),
    F('return', R_3, 'df.close / df.close.shift(i + 1)', '过去i个交易日的收益，=close_0/close_(i+1)', transform_for_ranker='x*10000'),
    F('rank_return', R_3, None, '过去i个交易日的收益 (return_i) 排名，=从小到大排名序号/总数', transform_for_ranker='x*10000'),

    F('amount', R_2, 'df.amount.shift(i)', '第前i个交易日的交易额', transform_for_ranker='x/10000'),
    ...
    F('turn', R_1, 'df.turn.shift(i)', '第前i个交易日的换手率', transform_for_ranker='x*10000'),
    ...
    H('pb_mrq', rank=True, transform_for_ranker='x.clip(-1000, 1000) * 10 + x/1000 + 20000', markets='US_STOCK,HK_STOCK'),
    ...
    'category:资金流',
    'markets:CN_STOCK_A',
    F('mf_net_amount', R_1, 'df.mf_net_amount.shift(i)', '第前i个交易日净主动买入额，= 买入金额 - 卖出金额 (包括超大单、大单、中单或小单)', transform_for_ranker=TRANSFORM_SLOG),
    F('avg_mf_net_amount', R_1, 'df.mf_net_amount.rolling(i).mean()', '过去i个交易日平均净主动买入额', transform_for_ranker=TRANSFORM_SLOG),
    F('rank_avg_mf_net_amount', R_1, 'df.mf_net_amount.rolling(i).mean()', '过去i个交易日平均净主动买入额排名', transform_for_ranker='x*10000'),
    ...
    H('fs_quarter_index', 'df.fs_quarter_index.replace(0, float("nan"))', '财报对应的季度，取值 1/2/3/4，1表示第一季度，以此类推', transform_for_ranker=None),
    ...
    'category:技术分析因子',
    'markets:all',
    F('ta_sma_$i_0', [5, 10, 20, 30, 60], 'MA(context.ta_input_data, timeperiod=i)', '收盘价的i日简单移动平均值', transform_for_ranker='x*10'),
    F('ta_ema_$i_0', [5, 10, 20, 30, 60], 'EMA(context.ta_input_data, timeperiod=i)', '收盘价的i日指数移动平均值', transform_for_ranker='x*10'),
    F('ta_wma_$i_0', [5, 10, 20, 30, 60], 'WMA(context.ta_input_data, timeperiod=i)', '收盘价的i日加权移动平均值', transform_for_ranker='x*10'),
    F('ta_ad', [0], 'AD(context.ta_input_data)', '收集派发指标', transform_for_ranker=TRANSFORM_SLOG),
    F('ta_aroon_down_$i_0', [14, 28], 'context.ta("AROON", i)[0]', '阿隆指标aroondown，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_aroon_up_$i_0', [14, 28], 'context.ta("AROON", i)[1]', '阿隆指标aroonup，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_aroonosc_$i_0', [14, 28], 'AROONOSC(context.ta_input_data, timeperiod=i)', 'AROONOSC指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_atr_$i_0', [14, 28], 'ATR(context.ta_input_data, timeperiod=i)', 'ATR指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_bbands_upperband_$i_0', [14, 28], 'context.ta("BBANDS", i)[0]', 'BBANDS指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_bbands_middleband_$i_0', [14, 28], 'context.ta("BBANDS", i)[1]', 'BBANDS指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_bbands_lowerband_$i_0', [14, 28], 'context.ta("BBANDS", i)[2]', 'BBANDS指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_adx_$i_0', [14,28], 'ADX(context.ta_input_data, timeperiod=i)', 'ADX指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_cci_$i_0', [14,28], 'CCI(context.ta_input_data, timeperiod=i)', 'CCI指标，timeperiod=i', transform_for_ranker='(x+1000)*100'),
    ...
    F('ta_obv', [0], 'context.ta("OBV")', 'OBV指标', transform_for_ranker='np.log(x)*100'),
    ...
    F('ta_mfi_$i_0', [14, 28], 'MFI(context.ta_input_data, timeperiod=i)', 'MFI指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_rsi_$i_0', [14, 28], 'RSI(context.ta_input_data, timeperiod=i)', 'RSI指标，timeperiod=i', transform_for_ranker='x*100'),
    F('ta_trix_$i_0', [14, 28], 'TRIX(context.ta_input_data, timeperiod=i)', 'TRIX指标，timeperiod=i', transform_for_ranker='x*100+10000'),
    F('ta_sar', [0], 'SAR(context.ta_input_data)', 'SAR指标', transform_for_ranker='x*10'),
    F('ta_mom_$i_0', [10, 20, 30, 60], 'MOM(context.ta_input_data, timeperiod=i)', 'MOM指标，timperiod=i', transform_for_ranker=TRANSFORM_SLOG),
    F('ta_willr_$i_0', [14, 28], 'WILLR(context.ta_input_data, timeperiod=i)', 'WILLR指标，timeperiod=i', transform_for_ranker='x * 100 + 200'),

    'category:波动率',
    'markets:all',
    H('swing_volatility_$i_0',  range=[5,10,30, 60, 120, 240], expr='((df.high - df.low)/df.close).rolling(center=False, window=i).std()', desc='振幅波动率，timeperiod=i',  rank=True, transform_for_ranker='x * 1000'),
    H('volatility_$i_0',  range=[5,10,30, 60, 120, 240], expr='(df.close/df.close.shift(1)-1).rolling(center=False, window=i).std()', desc='波动率，timeperiod=i',  rank=True, transform_for_ranker='x * 1000'),

    'category:BETA值',
    'markets:CN_STOCK_A',
    H('beta_sse50_$i_0',  range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("000016.SHA" ,timeperiod=i)', desc='BETA值(上证50)，timeperiod=i',  rank=True, transform_for_ranker='x * 100'),
    H('beta_csi300_$i_0', range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("000300.SHA" ,timeperiod=i)', desc='BETA值(沪深300)，timeperiod=i', rank=True, transform_for_ranker='x * 100'),
    H('beta_csi500_$i_0', range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("000905.SHA" ,timeperiod=i)', desc='BETA值(中证500)，timeperiod=i', rank=True, transform_for_ranker='x * 100'),
    H('beta_csi800_$i_0', range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("000906.SHA" ,timeperiod=i)', desc='BETA值(中证800)，timeperiod=i', rank=True, transform_for_ranker='x * 100'),
    H('beta_sse180_$i_0', range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("000010.SHA" ,timeperiod=i)', desc='BETA值(上证180)，timeperiod=i', rank=True, transform_for_ranker='x * 100'),
    H('beta_csi100_$i_0', range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("000903.SHA" ,timeperiod=i)', desc='BETA值(中证100)，timeperiod=i', rank=True, transform_for_ranker='x * 100'),
    H('beta_szzs_$i_0',   range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("000001.SHA" ,timeperiod=i)', desc='BETA值(上证综指)，timeperiod=i',rank=True, transform_for_ranker='x * 100'),
    H('beta_gem_$i_0',    range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("399006.SZA" ,timeperiod=i)', desc='BETA值(创业板)，timeperiod=i',  rank=True, transform_for_ranker='x * 100'),
    H('beta_industry_$i_0', range=[5, 10, 30, 60, 90, 120, 180], expr='context.beta("INDUSTRY" ,timeperiod=i)', desc='BETA值(所在行业)，timeperiod=i',  rank=True, transform_for_ranker='x * 100'),
])
