...
def key_pair_generation_point(d, x1, x2):

    point_g = Point(bytes_to_int(x1), bytes_to_int(x2))
...
def sm2_key_pair_gen_point(d, x1, x2):
    key_pair = key_pair_generation_point(d, x1, x2)
    prive_key = key_pair[0]
    pub_key = bytes_to_str(point_to_bytes(key_pair[1]))
...
def sm2_fromvkgetpk(vk):
    vkvalue = vk.hex().upper()
    vkvalue = "0x" + vkvalue.lstrip("0")
    b = hex_dec(vkvalue)

    key_pair = key_pair_generationbyvk(b)
    pub_key = bytes_to_str(point_to_bytes(key_pair[1]))

    a = get_a()
    b = get_b()
    point = bytes_to_point(a, b, str_to_bytes(pub_key))
    return fix_integer(point.x) + fix_integer(point.y)


def _x(x):
    x_ = 2 ** 127 + (bytes_to_int(x) & (2 ** 127 - 1))
    return x_
...
def gett(d, x, r):
    ...
    t = (bytes_to_int(d) + _x(x) * bytes_to_int(r)) % int(sm2_param_n, base=16)

    return list_to_bytes(int_to_bytes(t, 32))


# 计算椭圆曲线点  椭圆曲线点[_x1]RA
def getCurvePoint(x, y):
    private_key, public_key = sm2_key_pair_gen_point(_x(x), x, y)
    pubKey = com.aschex_to_bcdhex(public_key)
    return pubKey[0:32], pubKey[32:]


def key_pair_add(x1, y1, x2, y2):
    point1 = Point(bytes_to_int(x1), bytes_to_int(y1))
    point2 = Point(bytes_to_int(x2), bytes_to_int(y2))
    point_g = ECG_ele_add(point1, point2)

    pub_key = point_to_bytes(point_g)
    pk = b""
    if len(pub_key) == 65:
        pk = list_to_bytes(pub_key)[1:]
    else:
        pk = list_to_bytes(pub_key)
    return pk[0:32], pk[32:]


# 计算椭圆曲线点 V  或 U
def getCurvePointByt(t, x, y):
    private_key, public_key = sm2_key_pair_gen_point(t, x, y)
