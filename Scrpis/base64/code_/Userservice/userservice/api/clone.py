...
from ..settings import WEB_HOST,GATEWAY_HOST
...
async def create_clone_file(
...
) -> ResponseSchema[JupyterCloneSchema]:
    workspace_stats = WorkSpaceClient.get_all_workspace_status(credential)

    enable_workspace_list = [workspace.id for workspace in workspace_stats if workspace.status == 1]
    ...
    return ResponseSchema(data=f"/user/{credential.user.id.hex}/{enable_workspace_list[0]}/lab")
