...
# log = logbook.Logger('mlp_classifier')
log = BigLogger("mlp_classifier")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\分类"
bigquant_friendly_name = "多层感知器-分类"
bigquant_doc_url = ""
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = train(
        training_ds=training_ds, features=features, train_algorithm=MLPClassifier, train_parameters=dict(train_parameters, **other_train_parameters)
    )
    return outputs
