...
# log = logbook.Logger('cluster_agglomerative')
log = BigLogger("cluster_agglomerative")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\聚类"
bigquant_friendly_name = "层次-聚类"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    n_clusters: I.int("分类簇数量，默认为2", min=1) = 2,
    affinity: I.choice(
        "距离度量方法，度量的方法，包括cosine,manhattan,euclidean等", values=["euclidean", "l1", "l2", "manhattan", "cosine", "precomputed"]
    ) = "euclidean",
    linkage: I.choice("连接算法，如果使用ward，则距离度量只能使用euclidean", values=["ward", "complete", "average", "single"]) = "ward",
    ...
    if training_ds:
        ...
        if linkage == "ward" and affinity != "euclidean":
            raise Exception("连接算法使用使用ward时，距离度量只能使用euclidean")
        ...
        result_outputs = M.cached.v2(
            ...
            kwargs=dict(
                ...
                features=features,
                train_parameters=dict(n_clusters=n_clusters, affinity=affinity, linkage=linkage),
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = cluster_train(
        training_ds=training_ds,
        features=features,
        train_algorithm=AgglomerativeClustering,
