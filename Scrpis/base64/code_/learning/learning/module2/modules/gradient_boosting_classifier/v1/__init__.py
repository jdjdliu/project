...
# log = logbook.Logger('gradient_boosting_classifier')
log = BigLogger("gradient_boosting_classifier")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\分类"
bigquant_friendly_name = "梯度提升树-分类"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    loss: I.choice("损失函数，对于分类模型，有对数似然损失函数deviance和指数损失函数exponential", values=["deviance", "exponential"]) = "deviance",
    ...
    min_samples_per_leaf: I.int("树中终点节点所需要的最少的样本数，也可以用来防止过度拟合", min=1) = 1,
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = train(
        training_ds=training_ds,
        features=features,
        train_algorithm=GradientBoostingClassifier,
