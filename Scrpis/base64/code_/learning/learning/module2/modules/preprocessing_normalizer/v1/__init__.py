...
# log = logbook.Logger('preprocessing_normalizer')
log = BigLogger("preprocessing_normalizer")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\特征预处理"
bigquant_friendly_name = "标准化"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    norm: I.choice("标准化方法", values=["l1", "l2", "max"]) = "l2",
) -> [I.port("训练出来的模型", "output_model"), I.port("标准化训练集", "transform_trainds"), I.port("标准化预测集", "transform_predictds")]:
    """
    标准化处理。
    ...
    if training_ds:
        ...
        train_outputs = M.cached.v2(
            run=_transform, kwargs=dict(training_ds=training_ds, features=features, train_parameters=dict(norm=norm)), m_silent=True
        )
...
def _transform(training_ds, features=None, train_parameters={}, model=None):
    outputs = transform(training_ds=training_ds, features=features, train_algorithm=Normalizer, train_parameters=train_parameters, model=model)
    return outputs
...
if __name__ == "__main__":
    ...
    m2 = M.preprocessing_normalizer.v1(training_ds=m1.data_1, features=m3.data, predict_ds=m1.data_1, m_cached=False)
    print(1)
