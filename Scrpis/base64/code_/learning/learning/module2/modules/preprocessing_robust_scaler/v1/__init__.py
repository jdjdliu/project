...
# log = logbook.Logger('preprocessing_robust_scaler')
log = BigLogger("preprocessing_robust_scaler")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\特征预处理"
bigquant_friendly_name = "RobustScaler标准化"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    with_centering: I.bool("中心化") = True,
    with_scaling: I.bool("数据是否按分位数缩放") = True,
    quantile_range_min: I.float("  缩放分位数下限，介于0.0到100.0之间", min=0) = 25.0,
    quantile_range_max: I.float("缩放分位数上限，介于0.0到100.0之间", max=100) = 75.0,
    ...
    if training_ds:
        ...
        if not 0 <= quantile_range_min <= quantile_range_max <= 100:
            raise ValueError("请重新输入缩放分位数上下限，范围是0 <= quantile_range_min <= quantile_range_max <= 100")
        ...
        train_outputs = M.cached.v2(
            ...
            kwargs=dict(
                ...
                features=features,
                train_parameters=dict(with_centering=with_centering, with_scaling=with_scaling, quantile_range=quantile_range),
            ),
            m_silent=True,
        )
...
def _transform(training_ds, train_parameters={}, features=None, model=None):
    ...
    if not model:
        ...
        model = RobustScaler(**train_parameters)
    ...
    model_info = {
        "model": model,
        "features": features,
        "with_centering": model.with_centering,
        "with_scaling": model.with_scaling,
...
if __name__ == "__main__":
    ...
    m2 = M.preprocessing_robust_scaler.v1(
        training_ds=m1.data_1, features=m3.data, predict_ds=m1.data_1, quantile_range_min=0.0, quantile_range_max=90, m_cached=True
    )

    m3 = M.preprocessing_robust_scaler.v1(predict_ds=m1.data_1, model=m2.output_model, quantile_range_min=0.0, quantile_range_max=90, m_cached=True)
    print(1)
