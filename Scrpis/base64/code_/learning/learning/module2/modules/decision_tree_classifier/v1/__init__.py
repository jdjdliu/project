...
# log = logbook.Logger('decision_tree_classifier')
log = BigLogger("decision_tree_classifier")
bigquant_cacheable = True
# 模块接口定义
bigquant_category = r"机器学习\分类"
bigquant_friendly_name = "决策树-分类"
bigquant_doc_url = ""
...
def bigquant_run(
    ...
    criterion: I.choice("决定分割的标准，支持基尼不纯度gini和熵不纯度entropy，默认是gini", values=["gini", "entropy"]) = "gini",
...
def _train(training_ds, features, train_parameters, other_train_parameters):
    outputs = feature_train(
        training_ds=training_ds,
        features=features,
        train_algorithm=DecisionTreeClassifier,
