...
bigquant_friendly_name = "SeparableConv2D层"
...
def bigquant_run(
    ...
    strides: I.str("卷积的步长，strides，如为单个整数，则表示在各个空间维度的相同步长。任何不为1的strides均与任何不为1的dilation_rate均不兼容。用英文逗号(,)分隔的整数列表") = "1,1",
    padding: I.choice(
    ...
    depthwise_initializer: I.choice(
    ...
    user_depthwise_initializer: I.code(
        "自定义depthwise_initializer，示例：\ndef bigquant_run(shape, dtype=None):\n    import tensorflow.keras.backend as K\n    return K.random_normal(shape, dtype=dtype)",
        I.code_python,
    ) = None,
    pointwise_initializer: I.choice(
    ...
    user_pointwise_initializer: I.code(
        "自定义pointwise_initializer，示例：\ndef bigquant_run(shape, dtype=None):\n    import tensorflow.keras.backend as K\n    return K.random_normal(shape, dtype=dtype)",
        I.code_python,
    ) = None,
    ...
    depthwise_regularizer: I.choice("depthwise_regularizer，如果使用L1L2，可配置如下L1，L2参数", [DL.TEXT_REG_L1L2, DL.TEXT_NONE, DL.TEXT_USER]) = DL.TEXT_NONE,
    depthwise_regularizer_l1: I.float("depthwise_regularizer L1，如果使用L1L2正则化，此参数有效") = 0,
    depthwise_regularizer_l2: I.float("depthwise_regularizer L2，如果使用L1L2正则化，此参数有效") = 0,
    user_depthwise_regularizer: I.code(
        "自定义depthwise_regularizer，示例：\ndef bigquant_run(weight_matrix):\n    import tensorflow.keras.backend as K\n    return 0.01 * K.sum(K.abs(weight_matrix))",
        I.code_python,
    ) = None,
    pointwise_regularizer: I.choice("pointwise_regularizer，如果使用L1L2，可配置如下L1，L2参数", [DL.TEXT_REG_L1L2, DL.TEXT_NONE, DL.TEXT_USER]) = DL.TEXT_NONE,
    pointwise_regularizer_l1: I.float("pointwise_regularizer L1，如果使用L1L2正则化，此参数有效") = 0,
    pointwise_regularizer_l2: I.float("pointwise_regularizer L2，如果使用L1L2正则化，此参数有效") = 0,
    user_pointwise_regularizer: I.code(
        "自定义pointwise_regularizer，示例：\ndef bigquant_run(weight_matrix):\n    import tensorflow.keras.backend as K\n    return 0.01 * K.sum(K.abs(weight_matrix))",
        I.code_python,
    ) = None,
    ...
    depthwise_constraint: I.choice(
        "depthwise_constraint，在优化过程中为网络的参数施加约束", ["max_norm", "non_neg", "unit_norm", "min_max_norm", DL.TEXT_NONE, DL.TEXT_USER]
    ) = DL.TEXT_NONE,
    user_depthwise_constraint: I.code(
        "自定义depthwise_constraint，示例：\ndef bigquant_run(w):\n    from keras.constraints import max_norm\n    return max_norm(2.)(w)", I.code_python
    ) = None,
    pointwise_constraint: I.choice(
        "pointwise_constraint，在优化过程中为网络的参数施加约束", ["max_norm", "non_neg", "unit_norm", "min_max_norm", DL.TEXT_NONE, DL.TEXT_USER]
    ) = DL.TEXT_NONE,
    user_pointwise_constraint: I.code(
        "自定义pointwise_constraint，示例：\ndef bigquant_run(w):\n    from keras.constraints import max_norm\n    return max_norm(2.)(w)", I.code_python
    ) = None,
    ...
    depthwise_initializer = DL.smart_choice(depthwise_initializer, user_depthwise_initializer)
    pointwise_initializer = DL.smart_choice(pointwise_initializer, user_pointwise_initializer)
    bias_initializer = DL.smart_choice(bias_initializer, user_bias_initializer)
    depthwise_regularizer = DL.smart_regularizer_choice(
        depthwise_regularizer, user_depthwise_regularizer, depthwise_regularizer_l1, depthwise_regularizer_l2
    )
    pointwise_regularizer = DL.smart_regularizer_choice(
        pointwise_regularizer, user_pointwise_regularizer, pointwise_regularizer_l1, pointwise_regularizer_l2
    )
    ...
    depthwise_constraint = DL.smart_choice(depthwise_constraint, user_depthwise_constraint)
    pointwise_constraint = DL.smart_choice(pointwise_constraint, user_pointwise_constraint)
