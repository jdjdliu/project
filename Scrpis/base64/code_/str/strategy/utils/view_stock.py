from learning.module2.modules.derived_feature_extractor.v3 import BigQuantModule as derived_feature_extractor
from learning.module2.modules.general_feature_extractor.v7 import BigQuantModule as general_feature_extractor
from learning.module2.modules.instruments.v2 import bigquant_run as instruments
from sdk.datasource import DataSource
from sdk.strategy.constants import WeightMethod
from strategy.utils.notebook import generate_filter_cond_composition
...
class ViewStock:
    def __init__(
        ...
        self.instruments = instruments(start_date=self.start_date, end_date=self.end_date, market="CN_STOCK_A", instrument_list="")

    def features_list(self):
        features_base = list(FEATURE_MAP.keys())
        features = ["industry_sw_level1_0", "st_status_0", self.filter_cond] + list(self.sort_name.keys())
    ...
    def prepare_data(self):
        features = DataSource.write_pickle(self.features_list())
        general_feature = general_feature_extractor(instruments=self.instruments.data, features=features, start_date="", end_date="", before_start_days=0).run()
    ...
    def filter_data(self):
        factor_data = self.prepare_data().data.read()
        factor_data = factor_data[factor_data["filter_cond"]]
        if self.stock_pool != "全市场":
            factor_data = factor_data[factor_data[STOCK_POOL_MAP[self.stock_pool]] == 1]
        if self.market != ["全市场"]:
            factor_data = factor_data[factor_data.instrument.str.startswith(tuple(MARKET_MAP[x] for x in self.market))]
        if self.industry != ["全行业"]:
            factor_data = factor_data[factor_data.industry_sw_level1_0.isin([INDUSTRY_MAP[x] for x in self.industry])]
    ...
    def cal_score(self, df):
        ...
        for i in range(len(self.sort_weight)):
            df[sort_name_list[i] + "_score"] = df[sort_name_list[i]].rank(pct=True, method="max", ascending=self.sort_direction[i]) * 100
            df["total_score"] += df[sort_name_list[i] + "_score"] * self.sort_weight[i]
        df["total_score"] = df["total_score"].rank(pct=True, method="max", ascending=True) * 100
        return df

    def sort_data(self):
        factor_data = self.filter_data().read().round(2)
        sorted_data = factor_data.groupby("date").apply(self.cal_score)
        sorted_data = sorted_data.groupby("date").apply(lambda x: x.sort_values("total_score", ascending=False)).reset_index(drop=True)
        sorted_data = DataSource.write_df(sorted_data)
        return sorted_data

    def view_stock(self):
        sorted_data = self.sort_data().read()
        sort_name_list = list(self.sort_name.keys())
        sort_dict = {"total_score": "总得分"}
        for i in range(len(sort_name_list)):
            sort_dict[sort_name_list[i] + "_score"] = self.sort_name[sort_name_list[i]] + "得分"
        filter_dict = {"instrument": "股票代码", "date": "时间"}
        ...
        sorted_data = sorted_data[sorted_data.date == self.view_date][filter_dict_list]
...
# weight_method = "等权重"  # 全部选项为 "流通市值加权"、"总市值加权"、"等权重"
# market_index = "上证综指"
