...
class BuildStrategy:
    def __init__(self, start_date, end_date, filter_cond, filter_name, sort_name, sort_direction, sort_weight, drop_st_status, stock_pool, market, industry, rebalance_days, weight_method, market_index, select_time_parameters, is_select_time, view_date):
        ...
        self.date = DataSource("all_trading_days").read(
    ...
    def prepare_data(self):
        ...
        general_feature = M.general_feature_extractor.v7(
            instruments=self.instruments.data, features=features, start_date='', end_date='', before_start_days=0)
    ...
    def view_stock(self):
        ...
        sorted_data = sorted_data[sorted_data.date ==
                                  self.view_date][filter_dict_list]
...
def get_weight(df, n, weight_method):
    \"\"\"确定每只股票的权重\"\"\"
    pool = df.head(n)
...
def m3_handle_data_bigquant_run(context, data):
    ...
    index_data = context.index_data[
        context.index_data.date == data.current_dt.strftime('%Y-%m-%d')]
...
weight_method = '{weight_method}'  # 全部选项为 "流通市值加权"、"总市值加权"、"等权重"
